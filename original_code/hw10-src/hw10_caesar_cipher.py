######################################################################
# Author(s): Jayden Fleming
# Username(s): flemingj2
#
# Assignment: HW10: Caesar Cipher
#
...
####################################################################################
import hw10_image_converter as img_converter
...
class CaesarCipher:
    ...
    def decrypt(self):
        """
        Decrypts cipher via shifting letters left in alphabet by key.
...
class ImgCipher(CaesarCipher):
    def __init__(self, crypt_type, input_file="letters/message_input.txt", key=0):
        super().__init__(crypt_type, input_file="letters/message_input.txt", key=0)

    def decrypt(self, img_1, img_2):
        self.key = img_converter.img_r_diff(img_converter.ImageConverter(img_1), img_converter.ImageConverter(img_2))
        super().decrypt()
...
def main():
    """
    # Small program to demonstrate CaeserCipher class capabilities
    """
    ...
    cipher_lentulus.export_file("out_enc_lentulus")
    ...
    cipher_marcus.export_file("out_enc_marcus")
    ...
    cipher3.decrypt()
    cipher3.export_file("out_dec_friend3")
